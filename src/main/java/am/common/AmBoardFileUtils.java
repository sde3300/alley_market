...
//	매개변수로 boardIdx와 MultipartHttpServletRequest 타입의 변수를 넘겨 받음
	public List<BoardFileDto> parseFileInfo(String boardCategoryPk, int boardPk, MultipartHttpServletRequest mhsr) throws Exception {
...
//					위에서 확인한 파일의 정보를 DB에 저장하기 위해서 Board1FileDto 타입에 저장하기 위한 Board1FileDto 클래스 타입의 변수 boardFile 생성
					...
					boardFile.setBoardCategoryPk(boardCategoryPk);
...
//					ProductFile.setStoredFilePath(newFileName);
					boardFile.setStoredFilePath(storedPath + "/" + newFileName);
					
					if (boardCategoryPk.equals("b1")) {
						boardFile.setNoticePk(boardPk);
					}
					else if (boardCategoryPk.equals("b2")) {
						boardFile.setReviewPk(boardPk);
					}
					else {
						boardFile.setQnaPk(boardPk);
					}
